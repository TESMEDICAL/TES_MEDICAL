...
namespace TES_MEDICAL.GUI.Services
{
    public class ReportSvc : IReport
    {
        ...
        {
            var item = await _context.HoaDon.Include(x => x.MaNVNavigation).Include(x => x.MaPKNavigation).ThenInclude(x => x.MaBSNavigation).Include(x => x.MaPKNavigation).ThenInclude(x => x.MaBNNavigation)
